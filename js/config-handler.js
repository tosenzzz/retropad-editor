function ConfigHandler() {
  ...
  this.getConfigString = function () {
    _cleanUp();
    let img = document.getElementById('img-folder').value;
    if (!!img) {
      for (let i = 0; i < _strings.length; i++) {
        _strings[i] = _strings[i].replace(/=\s*(.*\.png)/, `= ${img}/$1`);
      }
    }
  ...
  this.setCurrentLineSectionValue = function (section, value) {
    ...
    _strings[_currentLine] = _editParamSection(
      _strings[_currentLine],
      section,
      value,
    );
  };
  ...
  this.setSelectionSectionValue = function (section, sValue) {
    ...
    if (selectedButtonLineIndexes.length == 0) return;
    ...
    for (let i = 0; i < selectedButtonLineIndexes.length; i++) {
      ...
      switch (section) {
        case 'x':
          ...
          _strings[index] = _editParamSection(
            _strings[index],
            section,
            confValue.toFixed(10),
          );
          break;
        case 'y':
  ...
  this.getCurrentOverlayBackground = function () {
    let bg = {};
    let fullscreen = _getParamValue(
      'overlay' + _currentOverlay + '_full_screen',
    );
  ...
  this.createOverlay = function (name, params) {
    if (this.isOverlayNameExist(name)) return;
  ...
  this.duplicateCurrentOverlay = function (name, params) {
    ...
    for (let i = 0; i < current.length; i++) {
      if (current[i].search('^' + overlayXX + '_desc') == -1) continue;
  ...
  this.getCurrentOverlayParams = function () {
    ...
    for (let i = 0; i < current.length; i++) {
      if (current[i].search('^' + overlayXX + '_desc') == 0) continue;
  ...
  this.createButton = function (command, shape, image, addLines) {
    ...
    if (!last) last = _getParamIndex(`${overlayXX}_descs`);

    if (last == -1) throw new Error('can not find position to insert new line');

    let overlayXX_descYY = `${overlayXX}_desc${buttCount}`;
    console.log('NEW BUTTON', overlayXX_descYY, command, shape, image);
    let arr = [
      `${overlayXX_descYY} = "${command},0.50000,0.50000,${shape},0.05000,0.05000"`,
    ];
  ...
  this.deleteCurrentButton = function () {
    ...
    let buttCount = Number(
      _getParamValue('overlay' + _currentOverlay + '_descs'),
    );
    ...
    let delNumber = Number(
      parameter.substr(('overlay' + _currentOverlay + '_desc').length),
    );
  ...
  this.fixAspect = function (iw, ih, ow, oh, portrait, keepRelativePositions) {
    ...
    if (portrait) coef = 1 / coef;

    for (let i = 0; i < _strings.length; i++) {
      let parameter = _isOverlayXX_descYY(_strings[i]);
      if (parameter) {
        if (keepRelativePositions) __scalePositioned(i, coef, axis);
        else __scaleSnapped(i, coef, axis);
      }
    }
  ...
  this.setCurrentOverlay = function (num) {
    ...
    let count = Number(_getParamValue('overlays'));
    if (num && num < count) _currentOverlay = num;
    else _currentOverlay = 0;
  };
  ...
  this.normalizeWidth = function (width, height) {
    ...
    if (selectedButtonLineIndexes.length > 0) {
      ...
      w = (
        (height / width) *
        _getParamSectionValue(_strings[_currentLine], 'h')
      ).toFixed(5);
  ...
  this.normalizeHeight = function (width, height) {
    ...
    if (selectedButtonLineIndexes.length > 0) {
      ...
      h = (
        (width / height) *
        _getParamSectionValue(_strings[_currentLine], 'w')
      ).toFixed(5);
  ...
  this.getCurrentButtonParams = function () {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (
        lines[i].search(param + '_') == 0 &&
        lines[i].search(param + '_overlay') == -1
      ) {
  ...
  this.updateCurrentButton = function (command, shape, image, addLines) {
    ...
    if (command.search('analog_') == 0) shape = 'radial';
  ...
  this.getOverlayAspectRatio = function () {
    ...
    if (ratio) return _calculateAspect(ratio);
  };

  this.setOverlayAspectRatio = function (coef) {
    if (coef && !isNaN(coef))
      _setParamValue(
        'overlay' + _currentOverlay + '_aspect_ratio',
        +Number(coef).toFixed(7),
      );
  };
  ...
  this.selectButtonsInBounds = function (left, top, right, bottom) {
    ...
    for (let i = 0; i < _strings.length; i++) {
      ...
      if (param) {
        ...
        if (x >= left && x <= right && y >= top && y <= bottom) indexes.push(i);
      }
    }
  ...
  function _cleanUp() {
    for (let i = _strings.length - 1; i >= 0; i--) {
      ...
      if (_strings[i].search(/.\d_overlay\s*=/) != -1) {
        ...
        if (quot && quot.length == 2) value = quot[1];

        // remove path
        let lastShalshIndex = Math.max(
          value.lastIndexOf('\\'),
          value.lastIndexOf('/'),
        );
        if (lastShalshIndex != -1) value = value.substr(lastShalshIndex + 1);

        // add quotemarks if spaces present in filename
        if (value.search(/\s/) == -1) _strings[i] = param + ' = ' + value;
        else _strings[i] = param + ' = "' + value + '"';
      }

      // surround overlay name with quotemarks
      if (
        _strings[i].search(/^overlay\d+_name\s*=/) == 0 ||
  ...
  function _getParamIndex(param) {
    for (let i = 0; i < _strings.length; i++)
      if (_strings[i].split('=')[0].trim() == param) return i;
  ...
  function _setParamValue(param, value) {
    ...
    if (index == -1) _insertParam(param, value);
    else _strings[index] = param + ' = ' + value;
  }
  ...
  function _deleteParamStrings(param) {
    ...
    for (let i = _strings.length - 1; i >= 0; i--)
      if (_strings[i].trim().search(reg) != -1) _strings.splice(i, 1);
  }
  ...
  function _getParamStrings(param) {
    ...
    let reg = new RegExp('^' + param + '([^0-9]|$)');
    for (let i = 0; i < _strings.length; i++)
      if (_strings[i].trim().search(reg) != -1) ret.push(_strings[i]);
  ...
  function _updateLinkedButtons(currentName, newName) {
    ...
    for (let i = _strings.length - 1; i >= 0; i--) {
      if (_strings[i].search(regParam) == -1) continue;

      if (_strings[i].search(regValue) == -1) continue;
  ...
  function _calculateAspect(coef) {
    ...
    for (let i = 1; i <= 24; i++)
      for (let j = 1; j <= 24; j++)
        if (coef.toFixed(5) == (i / j).toFixed(5)) return { w: i, h: j };
  ...
  function _normalizeOverlays(onFinshCallback, imagesObj) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (!__isOverlayNormalized(i)) {
        if (imagesObj[name]) {
          ...
          if (!missingImages.includes(name)) missingImages.push(name);
        }
      }
    }
    ...
    if (missingImages.length > 0) {
      alert(
        'Images are requiered for loading gamepad config:\n\n' +
          missingImages.join('\n') +
    ...
    if (toLoad == 0) {
      ...
      if (onFinshCallback) onFinshCallback();
