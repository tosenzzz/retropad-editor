...
const defaultParamsForNewOverlay =
  'full_screen = true\nnormalized = true\nrange_mod = 1.5\nalpha_mod = 2.0';
const autoScaleParams = 'auto_x_separation = true\n'; //auto_y_separation = ?
const manualScaleParams =
...
let screen = {
  ...
  get shortSide() {
    return Math.min(this._height, this._width);
  },

  set width(value) {
    this._width = Number(value || screen._width || DEF_WIDTH);
  },
  get enteredWidth() {
    return this.isPortrait ? this.shortSide : this.longSide;
  },
  get width() {
    return this.enteredWidth * this.scale;
  },

  set height(value) {
    this._height = Number(value || screen._height || DEF_HEIGHT);
  },
  get enteredHeight() {
    return this.isPortrait ? this.longSide : this.shortSide;
  },
  get height() {
    return this.enteredHeight * this.scale;
  },
...
document
  .getElementById('chk-show-shapes')
  .addEventListener('change', toggleShapes);
document
  .getElementById('chk-show-names')
  .addEventListener('change', toggleNames);
document
  .getElementById('chk-show-portrait')
  .addEventListener('change', toggleOrientation);
document
  .getElementById('chk-show-offscreen')
  .addEventListener('change', toggleOffscreen);
document
  .getElementById('overlay-selector')
  .addEventListener('change', selectOverlay);

document
  .getElementById('command-select')
  .addEventListener('change', fillCommandField);
document
  .getElementById('image-select')
  .addEventListener('change', fillImageNameField);
document
  .getElementById('image-name')
  .addEventListener('input', (e) => showImagePreview(e.target.value));

document
  .getElementById('load-config')
  .addEventListener('change', loadConfigFromFile);
document
  .getElementById('load-button-images')
  .addEventListener('change', loadImageFiles);
document
  .getElementById('load-screenshot')
  .addEventListener('change', loadScreenshotFile);
document
  .getElementById('chk-show-screenshot')
  .addEventListener('change', toggleScreenshot);
...
function createPadView() {
  ...
  for (let i = 0; i < rects.length; i++) {
    ...
    if (r.img) b.style['background-image'] = 'url(' + images[r.img] + ')';

    if (r.s == 'radial') b.classList.add('radial');

    b.dataset.lineIndex = r.i;
    if (conf.isLineInSelection(r.i)) b.classList.add('selected');

    b.addEventListener('click', () => {
      if (currentRect) currentRect.classList.remove('selected');
...
function createPadBackground() {
  ...
  let padFrame;
  if (bg.fullscreen) padFrame = document.getElementById('screenpad');
  else padFrame = document.getElementById('game-screenshot');
  ...
  function cancelSelection() {
    ...
    if (
      indexes.length == 0 &&
      conf.getCurrentLineSectionValue('shape') === null
    )
      enableEditor(false);
  ...
  padContianer.onmousemove = (event) => {
    if (event.buttons != 1 || !isMouseDown) return;
...
function getButtonsInRect(left, top, right, bottom, container) {
  ...
  let indexes = conf.selectButtonsInBounds(
    rectLeft,
    rectTop,
    rectRight,
    rectBottom,
...
function setEditorControls() {
  ...
  if (size) enableEditorSliders(true);
  else return;
...
function loadImageFiles(e) {
  ...
  for (let i = 0; i < e.target.files.length; i++) {
    ...
    if (!file || (ext != '.png' && ext != '.jpg')) continue;
...
function refreshScreenshot() {
  ...
  else shot.style['background-image'] = 'none';
}
...
function createRect(target, name, x, y, w, h, pct) {
  ...
  if (pct) {
    ...
    inner.style['background-image'] =
      'radial-gradient(transparent, rgba(100,100,200,0.4) ' +
      perc +
      '%, transparent ' +
      (perc + 1) +
      '%)';
...
function resetScreen() {
  ...
  if (screen.isPortrait) d.style.top = 0;
  else d.style.top = (screen.height - shotHeight) / 2 + 'px';
...
function setScreenDimensions(width, height, screenshotWidth, screenshotHeight) {
  ...
  if (aspect) ratio = aspect.w / aspect.h;

  // Reverse ratio if it does not match overlay name or orientation checkbox
  if ((screen.isPortrait && ratio > 1) || (!screen.isPortrait && ratio < 1))
    ratio = 1 / ratio;
...
function updateCurrentLine(section, value) {
  if (conf.getCurrentLineSectionValue('shape') === null) return;

  if (section) conf.setCurrentLineSectionValue(section, value);
...
function fillButtonEditor(command, shape, image, addLines) {
  ...
  document.getElementById('button-shape').selectedIndex =
    shape == 'rect' ? 0 : 1;

  if (image) document.getElementById('image-name').value = image;
  else document.getElementById('image-name').value = '';
...
function fillImageSelector() {
  ...
  let listAll = [];
  if (userImages.length > 0) listAll = listAll.concat(userImages);

  let defImages = [''];
  for (let f in images) if (!userImages.includes(f)) defImages.push(f);
...
function showAdditionalParametersForCommand(command) {
  let parameters = {
    ...
    get overlay_next() {
      ...
      else return 'next_target = ' + list[0];
    },
  ...
  data.forEach((e) => {
    ...
    try {
      fields = document.querySelectorAll(
        '.js-additional-button-property #' + prop + '_property',
      );
    } catch {
...
function clearAdditionalPropsFields() {
  let v = document.querySelectorAll(
    '.js-additional-button-property input, .js-additional-button-property select',
  );
...
function showDialog(elementId, isShow) {
  ...
  let focusCandidates = document.querySelectorAll(
    '#' + elementId + ' .js-dialog__focus',
  );
  if (focusCandidates.length > 0) focusCandidates[0].focus();
}

function showImagePreview(imgName) {
  let image = images[imgName];
  let gradient =
    'linear-gradient(90deg, rgba(0,0,0,0) 0%, rgba(0,0,0,0) 80%, #aac 90%)';
  let box = document.getElementById('image-name');

  if (image) box.style['background-image'] = 'url(' + image + '), ' + gradient;
  else box.style['background-image'] = 'none';
}
...
function toggleShapes(event) {
  ...
  if (event.target.checked) s.classList.add('show-borders');
  else s.classList.remove('show-borders');
}

function toggleNames(event) {
  let s = document.getElementById('screenpad');

  if (event.target.checked) s.classList.remove('hide-names');
  else s.classList.add('hide-names');
}
...
function fixAspect() {
  ...
  if (ow >= 96 && oh >= 64) setScreenDimensions(ow, oh);
...
function addOverlay() {
  ...
  else conf.createOverlay(name, props);
...
function delCurrentButton() {
  ...
  if (!conf.deleteCurrentButton()) alert('No selection!');
  redrawPad();
}
...
function showAspectFixer() {
  ...
  if (aspect) {
    ...
    document.getElementById('initial-aspect-width').value = screen.isPortrait
      ? 9
      : 16;
    document.getElementById('initial-aspect-height').value = screen.isPortrait
      ? 16
      : 9;
  }

  let hint = document.getElementById('aspect-hint');
  if (screen.isPortrait) hint.classList.remove('hidden');
...
function onScreenshotModeChange() {
  ...
  let disableSizeSet =
    screen.shotImage &&
    screen.shotShow &&
    (screenshotFit.checked || screenshotMatch.checked);
...
function toggleAdditionalButtonProperties(show) {
  ...
  if (show || adds[0].classList.contains('hidden')) {
    ...
    for (let i = 0; i < adds.length; i++) adds[i].classList.remove('hidden');
  } else {
    addBtn.classList.remove('expanded');
    for (let i = 0; i < adds.length; i++) adds[i].classList.add('hidden');
  }
}
...
function updateNewOverlayFields() {
  ...
  if (isEdit) toggleAdditionalOverlayProperties(true);
  ...
  if (isDuplicate) {
    ...
    let ratio =
      'aspect_ratio = ' + +(isPortrait ? 1 / aspect : aspect).toFixed(7);
  ...
  function _fillCurrentOverlay() {
    ...
    isPortrait =
      document.getElementById('overlay-selector').value.search('portrait') !=
      -1;
...
function selectOverlay(event) {
  ...
  if (event.target.value.search('portrait') != -1) screen.isPortrait = true;

  if (event.target.value.search('landscape') != -1) screen.isPortrait = false;
...
function setColorScheme(index) {
  ...
  if (index > 0) screenpad.classList.add('scheme-' + index);
}
